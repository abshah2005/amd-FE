...
const demoActiveQuestions = [
  { text: "Looking for guidance on how to structure my investor pitch.", daysLeft: 5 },
  { text: "Looking for guidance on how to structure my investor pitch.", daysLeft: 10 },
  { text: "Looking for guidance on how to structure my investor pitch.", daysLeft: 12 },
];
      ...
      <ul className="text-sm space-y-6">
        {items.map((it, i) => (
          <li key={i} className="flex items-start gap-3">
            ...
            <div>
              <div className="text-xs text-gray-500">{it.daysLeft} days remaining</div>
    ...
    <div className="flex items-center gap-4">
      {["Active", "Archived"].map((t) => (
        <button
          ...
          onClick={() => setActive(t)}
          className={`text-sm ${
            active === t ? "text-blue-600 border-b-2 border-blue-600 pb-2" : "text-gray-600"
          }`}
        >
          {t}
        </button>
      ...
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="flex gap-6 items-start mb-6">
          <div className="grid grid-cols-4 gap-6 flex-1">
            <StatsCard title="Active Questions" value={demoStats.activeQuestions} />
            <StatsCard title="Question Completed" value={demoStats.questionsCompleted} />
            <StatsCard title="Avg. Rating" value={demoStats.avgRating} />
            <StatsCard title="Total Earnings" value={demoStats.totalEarnings} />
          </div>
        ...
        <div className="grid grid-cols-4 gap-6">
          ...
          <main className="col-span-3">
            <h2 className="text-2xl font-semibold mb-4">Pending Actions</h2>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {filtered.map((row) => (
                        <tr key={row.id} className="hover:bg-gray-50">
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-blue-600 font-medium">{row.id}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{row.submittedDate}</td>
                          <td className="px-6 py-4 text-sm text-gray-700 max-w-lg truncate">{row.question}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{row.asker}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{row.price}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm"><StatusCell status={row.status} /></td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{row.deliveryTime}</td>
                        </tr>
                      ))}
